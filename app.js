  ...
  return {
    ...
    var simulation = d3.forceSimulation(nodes)
        ...
        .force("link", d3.forceLink(links)
                          .distance(d => {
                            var count1 = d.source.commitees.length;
                            var count2 = d.target.commitees.length;
    ...
    function makeLinks(nodes) {
      ...
      for (var i = 0; i < nodes.length; i++) {
        for (var j = 1 + j; < nodes.length; j++) {
          ...
          var commitee = s1.commitees[k];
          if (s2.commitees.includes(commitee)) {
            links.push({
              source: s1.name,
              target: s2.name,
            });
